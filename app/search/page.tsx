  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="max-w-7xl mx-auto px-6 md:px-8 py-16 md:py-20 border-b border-border">
        ...
        {summary && (
          <p className="text-muted-foreground text-lg max-w-2xl leading-relaxed italic" style={{ fontFamily: "var(--font-display)" }}>
            {summary}
          </p>
        )}
      </header>
        ...
        {venues.length > 0 ? (
              ...
              {venues.map((venue) => (
                <article key={venue.id} className="group bg-surface hover:bg-background transition-colors duration-300">
                  ...
                  <div className="p-6 md:p-8">
                    ...
                    <h2 className="font-display font-light text-foreground text-xl md:text-2xl mb-2 leading-snug group-hover:text-primary transition-colors" style={{ fontFamily: "var(--font-display)" }}>
                      {venue.name}
                    </h2>
                    {venue.address && (
                    ...
                    {venue.description && (
                      <p className="text-muted-foreground text-sm leading-relaxed line-clamp-2 mb-4">{venue.description}</p>
                    )}
                    <div className="flex items-center justify-between pt-4 border-t border-border">
                      ...
                      <Link href={`/venue/${venue.id}`} className="link-luxury text-[10px]">Voir la fiche →</Link>
                    </div>
                  </div>
                </article>
              ))}
          ...
          <div className="py-24 text-center">
            <p className="font-display font-light text-foreground text-3xl mb-4" style={{ fontFamily: "var(--font-display)" }}>
              {query ? `Aucun résultat pour « ${query} »` : "Saisissez votre recherche"}
            </p>
            <p className="text-muted-foreground mb-8">
              {"Essayez «\u00a0Montréal gastronomie\u00a0» ou «\u00a0bar à cocktails Québec\u00a0»"}
            </p>
            <Link href="/" className="link-luxury">Retour à l'accueil →</Link>
          </div>
        )}
      </main>
