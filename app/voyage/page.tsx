...
const destinations = [
  {
    id: "v1", label: "Escapade hivernale", name: "Vieux-Québec",
    subtitle: "La ville fortifiée, patrimoine de l'UNESCO", location: "Québec City, QC", duration: "Week-end",
  ...
  {
    id: "v2", label: "Gastronomie & Nature", name: "Charlevoix",
    subtitle: "Réserve mondiale de la biosphère", location: "Charlevoix, QC", duration: "3–4 jours",
  ...
  {
    id: "v3", label: "Vignobles", name: "Estrie",
    subtitle: "Cantons-de-l'Est — vignobles et auberges", location: "Estrie, QC", duration: "Week-end",
  ...
  {
    id: "v4", label: "Chute spectaculaire", name: "Chutes Montmorency",
    subtitle: "Plus hautes que les chutes Niagara", location: "Beauport, QC", duration: "Journée",
  ...
  {
    id: "v5", label: "Île & terroir", name: "Île d'Orléans",
    subtitle: "Le jardin de Québec", location: "Île d'Orléans, QC", duration: "Journée",
  ...
  {
    id: "v6", label: "Grand Nord", name: "Laurentides",
    subtitle: "Ski, randonnée et chalet de luxe", location: "Laurentides, QC", duration: "3–5 jours",
...
const transport = [
  { icon: Train, name: "Train — Via Rail", route: "Montréal → Québec", time: "3h30", price: "50$ – 150$", note: "Recommandé — Vue sur le Saint-Laurent", href: "https://www.viarail.ca" },
  { icon: Car, name: "Voiture", route: "Autoroute 20 ou 40", time: "2h45", price: "30$ – 60$", note: "Flexibilité maximale avec arrêts", href: "https://www.rentalcars.com" },
  { icon: Bus, name: "Autobus — Orléans Express", route: "Montréal → Québec", time: "3h45", price: "40$ – 80$", note: "Option économique, WiFi à bord", href: "https://www.orleansexpress.com" },
]
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-8 py-14 md:py-16 space-y-16 md:space-y-20">
            ...
            {destinations.filter((d) => d.featured).map((dest, i) => (
              ...
              >
                ...
                <div className="bg-surface p-10 md:p-14 flex flex-col justify-center border-t border-border md:border-t-0 md:border-l border-border">
                  ...
                  <p className="text-muted-foreground leading-relaxed mb-8">{dest.description}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-primary" />{dest.location}</span>
                      <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-primary" />{dest.duration}</span>
                    </div>
            ...
            {destinations.filter((d) => !d.featured).map((dest) => (
              ...
              >
                ...
                <div className="p-6 md:p-8">
                  ...
                  <h3 className="font-display font-light text-foreground text-xl md:text-2xl mb-1 leading-snug group-hover:text-primary transition-colors duration-300" style={{ fontFamily: "var(--font-display)" }}>{dest.name}</h3>
                  <p className="text-primary text-xs italic mb-4" style={{ fontFamily: "var(--font-display)" }}>{dest.subtitle}</p>
                  <p className="text-muted-foreground text-sm leading-relaxed line-clamp-2 mb-5">{dest.description}</p>
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <Clock className="w-3 h-3 text-primary" />
                      <span>{dest.duration}</span>
                    </div>
                    <span className="text-[10px] tracking-[0.18em] uppercase text-primary font-sans hover:text-primary-dark transition-colors">Explorer →</span>
                  </div>
                </div>
              </article>
            ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground mb-3">
                    <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-primary" /> {t.time}</span>
                    <span className="text-foreground font-medium">{t.price}</span>
                  </div>
                  <p className="text-muted-foreground text-xs leading-relaxed flex-1">{t.note}</p>
                  <div className="mt-6 link-luxury inline-block">Réserver →</div>
                </a>
              )
            })}
          </div>
        ...
        {/* Dark CTA */}
        <section className="full-bleed bg-foreground py-20">
          <div className="max-w-3xl mx-auto px-6 text-center">
            <p className="text-[10px] tracking-[0.22em] uppercase text-primary font-sans mb-6">Membership</p>
            <h2 className="font-display font-light text-primary-foreground text-3xl md:text-4xl mb-6" style={{ fontFamily: "var(--font-display)" }}>
              Accédez aux itinéraires<br /><em className="italic text-primary">exclusifs de la maison</em>
            </h2>
